...
//  cout << "        background-color: #226;\n";
  ...
  cout << "        box.src = boxhost + '/DASH/SWITCH';\n";
  cout << "      }\n";
      ...
      if ((varsplit = varname.find("=", 0)) != string::npos) { // = means set a value
        ...
        while (++vari < varname.length()) {
          if (varname.at(vari) == '/') {
            ...
            isuppercase = false;
            cout << "ONLY UPPERCASE VARS ARE MUTABLE\n";
            cout << "PROTECTEDVARNAME: " << varname << "\n";
            cout << "REJECTEDVARVALUE: " << varvalue << "\n";
            break; // return 0;
          }
        ...
        if (isuppercase) { // NOTE: THE CURRENT FILE IS BEING OVERWRITTEN
        ...
        if (!writefailed && !readfailed) {
          ...
          cout << "FIXEDVALUE: " << uppercasevalue << "\n";
        ...
        bool readfailed = false;
        string uppercasevalue = "";
        if (isuppercase) { // NOTE: THE CURRENT FILE IS BEING READ
          CCS uppercasefilename = varname.c_str();
